	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\begin{itemize}
				...
				\item liší se podle zprávy, $0$ nebo $1$
			\end{itemize}
		\end{itemize}
		\pause
...
\frametitle{Využití nástrojů SCA k útoku na WBAES}
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item není jasné, kudy informace uniká
		\end{itemize}
		\pause
		\item paměťové stopy
		\begin{itemize}
...
\frame[plain]{
	\begin{table}[H]
		...
		\end{center}
	\caption{Pořadí správného kandidáta, $\blacksquare\sim 0$. Útok na WBAES s~použitím $1024$ stop. Průměrně $2.9$ terčů z $8$ uspěje ($36\%$).}
	\end{table}
}
...
\frame[plain]{
	\begin{table}[H]
		...
		\end{center}
	\caption{Pořadí správného kandidáta, $\blacksquare\sim 0$. Útok na WBAES s~použitím $1024$ stop. Průměrně $2.4$ terčů z $8$ uspěje ($30\%$).}
	\end{table}
}
...
\frame{
\frametitle{Úspěšnost dle invertibility $p$}
	\begin{figure}[h]
	\begin{center}
		\includegraphics[width=\textwidth]{../figures/leak_target/plot.pdf}
...
\frame{
\frametitle{Úspěšnost podle pozice bitu ve stopě}
	\begin{figure}[h]
	\begin{center}
		\includegraphics[width=\textwidth]{../figures/leak_bit/all.pdf}
	\caption{Průměrná úpěšnost a její směrodatná odchylka napříč instancemi podle pozice ve stopě$\mod{8}$ (tj.\ bit v rámci bytu).}
	\end{center}
	\end{figure}
}
...
\frametitle{Útok \uv{naslepo}}
	...
	\begin{itemize}
		...
		\end{itemize}
		\item průměr správných $34\%$
	\end{itemize}
	\pause
	Návrh
	\begin{itemize}
		\item použití menšího počtu stop a více terčů
...
\frametitle{Budoucí práce}
	Nedořešené otázky
	\begin{itemize}
		\item ze které tabulky dochází k úniku,
		\item jak útok teoreticky podložit,
		\item jak provést útok rovnou z tabulek,
