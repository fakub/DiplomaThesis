...
\subsubsection{Processing Results}
	...
	See the following table for partial results of a real experiment, further comments below.
		...
		\label{tab:gap}
		\end{table}
		According to the original criterion (i.e.\ picking the candidate with the highest value) we would get {\tt 09} in the \nth{4} bit column, but it is not the correct one -- in this experiment, the true candidate does not have the highest value.
	...
	Applying the rule from the previous remark to the previous example, we find the maximal gap for the \nth{3} target bit, its absolute value is $0.0330$ compared to $0.0086$, $0.0178$ and $0.0209$, its relative value is $9.5\%$ compared to $2.2\%$, $5.3\%$ and $5.6\%$. Hence the best candidate with this rule is {\tt 15} which is the correct one.   %!% implementovat
