...
\subsubsection{Processing Results}
	...
	\begin{example}
	\label{ex:gap}
	See the following table for partial results of a real experiment, further comments are below.
		\begin{table}[H]
			\begin{center}
			\begin{tabular}{| c | c | c | c | c | c | c | c | c |}
				\hline
				\multirow{2}{*}{Rank} & \multicolumn{2}{c|}{3. bit} & \multicolumn{2}{c|}{4. bit} & \multicolumn{2}{c|}{6. bit} & \multicolumn{2}{c|}{7. bit} \\
				\cline{2-9}
			...
			\end{tabular}
			\end{center}
		\caption{Partial results of a real experiment. The true key byte is {\tt 15} and is emphasized with brackets.}
		\label{tab:gap}
		\end{table}
		According to the original criterion (i.e.\ picking the candidate with highest value) we would get {\tt 09} which is not the correct one, in this experiment the true candidate does not have the highest value.
		
		But considering the previous observation, we can modify our criterion as follows: we choose that candidate which is on the top of any list and has the highest gap to the second candidate. With this rule we find the maximal gap for $3.$ target bit, its value is $0.0330$. Hence the best candidate with this rule is {\tt 15} which is the correct one.   %!% czenglish: nabývá? místo reached...
	\end{example}
	
	\begin{remark}
	\label{rem:gap}
		Note that other empirical criterions can be introduced, for example sum of gaps to the next candidate.   %!% mrdka, má bejt jenom pro první místa nějakýho listu! jinak nedává smysl, e.g. 0.43, 0.42, 0.40, 0.32, 0.31, ...
		%!% implementovat
	\end{remark}
	
	Note that we do not consider absolute values anymore, only the difference to the second candidate. 
