...
This property seems to be exactly what we want from a secure cipher but the following theorem shows that we want far too much.

\begin{thm}
	Let $(E,D)$ be a perfect cipher. Then $|K| \geq |M|$.
\end{thm}

% bla bla že to je fakt blbý aby byl klíč stejně dlouhej?
...
Based on the previous definitions formalizing the idea of the game, we can define another notion of security.
...
%~ Now we introduce an interesting approach to cipher's security. Before we give the formal definition, let us us start with another game similar to \ref{game:semsec}.
...
In a manner analogous to Definition \ref{def:ssadvant}, attacker's advantage is defined as follows.
...
PRP advantage tells us how likely adversary $A$ is able to distinguish a result of a PRP with a random key from a truly random bijection. Note that this is the same problem as introduced in Game \ref{game:prp}.   %!% ale trochu formálnějc
