...
\begin{defn}[Single-Access Oracle]   %!% NEW
\label{def:saoracle}
	Let $F:X\rarr Y$. {\em Single-access oracle evaluating $F$} is an oracle evaluating $F$, where the number of queries is limited to one; denoted by ${\cal O}^{(1)}_F$.
\end{defn}
...
\begin{defn}[Single-Access Advantage]   %!% NEW
\label{def:saadvant}
	{\em Single-access advantage} is defined as advantage, where single-access oracles are used instead; denoted by $\Adv^{(1)}$.
\end{defn}
