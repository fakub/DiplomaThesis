...
%~ \begin{defn}[Single-Access Oracle]
%~ \label{def:oracle}
	%~ Let $F:X\rarr Y$. {\em Oracle evaluating $F$} is a device which, given $x\in X$, evaluates and returns $F(x)$ in unit time; denoted by ${\cal O}_F$.
%~ \end{defn}
...
\label{def:ssadvant}
	Let $E: K\times M\rarr C$ be an encryption function of a symmetric cipher, $k\unirand K$, ${\cal E}_b = \{E_b:M^2\rarr C \mid E_b(m_0,m_1) = E(k,m_b), k\in K\}$ for $b=0,1$, and $A$ an adversary. We define {\em semantic security advantage} of adversary $A$ as
	\[
...
\label{note:singleaccess}
	PRP has no ambiguity, hence we will need to either limit the amount of adversary-challenger queries to one, or, equivalently, change the key after each query. If we did not do so, then there would obviously exist a trivially winning adversary as outlined in Note .   %!%
	
	Indeed, such an adversary would pick three distinct plaintexts $m_0$, $m_1$, $m_2$ (well, let us suppose that there are at least three plaintexts in $M$) and sends $(m_0,m_1)$ to the challenger, while she gets back some ciphertext $c_a$. In her second query, the adversary sends $(m_0,m_2)$ to the challenger and receives another ciphertext, say $c_b$. Since PRP is a deterministic bijection, she can decide with certainty:
	\begin{itemize}
...
\label{game:semsecprp}
	There are two parties -- a {\em challenger} and an {\em adversary}. Let further $E:K\times X\rarr Y$ be a PRP. First, the challenger chooses a random bit $b\unirand\{0,1\}$ and, before each round, also a random key $k\unirand K$. In each round, the adversary sends two equally long plaintexts $m_0$, $m_1$ of her choice to the challenger. The challenger computes $c = E(k,m_b)$ and sends it back to the adversary. Note that the adversary is allowed to perform several queries again, but, unlike the previous games, the key changes over and over again.
	
	The aim of the adversary is to distinguish effectively which $b$ has been chosen by the challenger.
\end{game}

%!% nahradit opravdickejma definicema
%~ \begin{note}
%~ \label{note:semsecprp}
	%~ Based on the previous game, we could define {\em PRP semantic security advantage} ($\AdvPRPSS$) and {\em semantically secure PRP} (cf. Game \ref{game:semsec}, Definition \ref{def:ssadvant} and \ref{def:semsec}). The difference would be that the oracle access should be redefined -- it would either need to be limited to one, or the oracle would need to change the function it evaluates. Due to these technical obstacles we do not provide exact definitions.
%~ \end{note}
