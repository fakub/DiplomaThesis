...
Let us first introduce some notion for the feature \ref{item:wrap} from the previous list as defined by Chow et al.\ \cite{chow2003aes}.
...
\subsubsection{Inserting Encodings}
	...
	Let us demonstrate previous ideas in a single sketch of the flow during one inner round (i.e. not the first nor the last) showing all operations, table compositions, mixing bijections and encodings. Tables are given their names (e.g.\ Type II) as introduced by Chow et al.\ \cite{chow2003aes}.
	\begin{align}
...
\subsubsection{BGE Attack}
	...
	\begin{note}
	\label{note:reverse}
		In a real-world scenario, we first need to reverse-engineer provided binary code in order to access the tables. This can make any analysis really painful -- such programs are often further protected against reverse engineering effort with various techniques.
	\end{note}
	
	We will not cover BGE attack in detail here, it is out of the scope of this thesis, but it motivated further research. Among others (e.g.\ \cite{michiels2007cryptographic} or \cite{xiao2009secure}), we especially highlight Karroumi's contribution \cite{karroumi2011protecting} which has been shown by Klinec \cite{klinec2013white} to be equivalent to the original WBAES by Chow et al. This will later serve as a theoretical fundament of our contribution.
...
\subsubsection{Karroumi's Approach with Dual Ciphers}
	...
	Karroumi incorporated so called {\em dual ciphers}.
	

\subsubsection{Equality Result}
	
	Klinec proved something important about Karroumi's enhancement.
