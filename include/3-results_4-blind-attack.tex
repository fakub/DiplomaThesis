...
\label{rem:false}
	...
	In our results, the target bits keep their original order, therefore we can deduce the row $B$ generating the target in terms of $T_B$. Here it is the last but one row of matrix of multiplication by $\texttt{3d}\pmod{x^8+1}$, see Example \ref{ex:shiftmatrix}. Hence the row is $B = 11110010$.
\end{remark}
